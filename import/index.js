...
export function configure ({ cdn: url, modules } = {}) {
...
function serializeOptions (options) {
  ...
  for (const key of Object.keys(options).sort()) {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      out += `${key}=${typeof value === 'function' ? value.name : value};`;
    }
  }
  return out;
}
...
export async function importYAML (path, options = {}) {
  ...
  const YAML = (await vendor('sass' in {} ? 'sass' : 'yaml')).default;
...
export async function importFile (path, options) {
  ...
  const cacheKey    = `import:${path}:${fingerprint}`;
